...
public class TilemapGenerator : MonoBehaviour
{
    ...
    {
        string assetPath = $"{folderPath}/{prefabFolder}/testPrefab[{index / mapSize.x},{index % mapSize.x}].prefab";
    ...
    private Map CreateMaps(GameObject prefab, int index)
    {
        ...
        string assetPath = $"{folderPath}/{mapFolder}/testMap[{index / mapSize.x},{index % mapSize.x}].asset";
    ...
    private void SetupFolders()
    {
        ...
        try
        {
            ...
            foreach (var item in currentTilemaps)
            {
                mapList.maps.Add(CreateMaps(prefabs[i], i));
                i++;
            }
        ...
        gameManagerScript.mapList = maplist;
    }
    #endregion
            ...
            if (currentTilemaps[i].TryGetComponent<CompositeTilemap>(out CompositeTilemap comp))
            {
                ...
                for (int j = comp.compMapXPos.x; j < comp.compMapXPos.y + 1; j++)
                {
                    for (int k = comp.compMapYPos.x; k < comp.compMapYPos.y + 1; k++)
                    {
                        ...
                        comp.connectedTilemaps.Add(gameManagerScript.mapList.maps[(j * mapSize.x) + k]);
                        compPrefab.connectedTilemaps.Add(gameManagerScript.mapList.maps[(j * mapSize.x) + k]);
    ...
    private void RecheckDoorsForCompositeMaps()
    {
        ...
        for (int i = 0; i < currentTilemaps.Count; i++)
        {
            if (currentTilemaps[i].TryGetComponent<CompositeTilemap>(out CompositeTilemap comp))
            {
                ...
                foreach (var item in gameManagerScript.mapList.maps[i].mapDoors)
                {
                    Debug.Log($"1. {item}");
                }

                if (comp.connectedTilemaps.Contains(gameManagerScript.mapList.maps[temp[0]]))
                    gameManagerScript.mapList.maps[i].mapDoors.Remove(Direction.Down);

                if (comp.connectedTilemaps.Contains(gameManagerScript.mapList.maps[temp[1]]))
                    gameManagerScript.mapList.maps[i].mapDoors.Remove(Direction.Up);

                if (comp.connectedTilemaps.Contains(gameManagerScript.mapList.maps[temp[2]]))
                    gameManagerScript.mapList.maps[i].mapDoors.Remove(Direction.Left);

                if (comp.connectedTilemaps.Contains(gameManagerScript.mapList.maps[temp[3]]))
                    gameManagerScript.mapList.maps[i].mapDoors.Remove(Direction.Right);
